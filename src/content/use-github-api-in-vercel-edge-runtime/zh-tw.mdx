...
我知道 Edge
Computing 也是近一年的事。~說實話，除非真的要用，沒人想去做 Web 前端~
...
我想做一些統一的 API，方便我的軟體自我檢查更新（很多人都直接把 GitHub API
hardcode 在軟體裡面，我覺得不夠優雅）。

Vercel 提供了一些
[Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions)，免費額度是五十萬次請求，差不多夠了。
...
根據 Vercel 的
[說明文件](https://vercel.com/docs/concepts/functions/edge-functions/quickstart)，可以不用自家的 Next.js，但腳本都要放在 /api 目錄裡。
...
我根據
[另一些 Vercel 的說明文件](https://vercel.com/docs/concepts/projects/project-configuration)，用
`vercel.json` 重新導向了網站的 URL：
...
整個 Edge Function 都是基於 ES Module 的；如果想匯入一些不是或不標準的 ES
Module：
...
octokit 和 query-string 這兩個 module 都可以跑在瀏覽器上，但因為沒有標註自己可以跑在瀏覽器上（也就是不是標準的 ES
Module），部署失敗了。
